...
from Screens.BhUtils import nab_strip_html, DeliteGetSkinPath
...
#        if self.tcat != 'Skins' and self.tcat != 'Logos':
        if getBoxType() in ('dm800'):
            nobox = self.session.open(MessageBox, _('Sorry, the preview is not available for Your Box.'), MessageBox.TYPE_INFO)
        else:
